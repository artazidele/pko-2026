
var chart_bestScoreDistributionSummaryChart1_5bd79 = new Chart(document.getElementById('chart_bestScoreDistributionSummaryChart1_5bd79'), {
    type: 'boxplot',
    data: {
        labels: [
            'example-data-1', 'example-data-2', 'example-data-3', 'example-data-4', 'example-data-5'
        ],
        datasets: [
                {
                    label: 'Hill Climbing (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Tabu Search 500 - 1000 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Tabu Search 500 - 5000 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Tabu Search 1000 - 5000 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Simulated Annealing 0-1000 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Simulated Annealing 1-5000 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Simulated Annealing 2-1000-10000 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Late Acceptance 200 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Late Acceptance 400 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }, 
                {
                    label: 'Late Acceptance 600 (favorite)',
                        borderWidth: 4
,
                    data: [
                                {
                                    min: 0,
                                    max: 0,
                                    q1: 0,
                                    q3: 0,
                                    median: 0,
                                    mean: 0,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -11185,
                                    max: -11185,
                                    q1: -11185,
                                    q3: -11185,
                                    median: -11185,
                                    mean: -11185,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -83714,
                                    max: -83714,
                                    q1: -83714,
                                    q3: -83714,
                                    median: -83714,
                                    mean: -83714,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -160731,
                                    max: -160731,
                                    q1: -160731,
                                    q3: -160731,
                                    median: -160731,
                                    mean: -160731,
                                    items: [],
                                    outliers: [],
                                }
                            , 
                                {
                                    min: -254743,
                                    max: -254743,
                                    q1: -254743,
                                    q3: -254743,
                                    median: -254743,
                                    mean: -254743,
                                    items: [],
                                    outliers: [],
                                }
                            
                    ]
                }
        ]
    },
    options: {
        animation: false,
        responsive: true,
        maintainAspectRatio: false,
        plugins: {
            title: {
                display: true,
                text: 'Best medium score distribution summary (higher is better)'
            }
        },
        scales: {
            x: {
                display: true
            },
            y: {
                title: {
                    display: true,
                    text: 'Best medium score'
                },
                display: true
            }
        },
watermark: {
    image: "website/img/timefold-logo-stacked-positive.svg",
    x: 15,
    y: 15,
    width: 48,
    height: 50,
    opacity: 0.1,
    alignX: "right",
    alignY: "bottom",
    alignToChartArea: true,
    position: "front",
}    },
plugins: [{ 
    id: 'customPlugin',
    beforeDraw: (chart, args, options) => {
          const ctx = chart.canvas.getContext('2d');
          ctx.save();
          ctx.globalCompositeOperation = 'destination-over';
          ctx.fillStyle = 'white';
          ctx.fillRect(0, 0, chart.canvas.width, chart.canvas.height);
          ctx.restore();
    }
}]
});

window.addEventListener('beforeprint', () => {
  chart_bestScoreDistributionSummaryChart1_5bd79.resize(1280, 720);
});
window.addEventListener('afterprint', () => {
  chart_bestScoreDistributionSummaryChart1_5bd79.resize();
});